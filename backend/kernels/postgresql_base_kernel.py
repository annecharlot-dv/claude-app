...
class PostgreSQLBaseKernel(ABC):
    ...
    async def create_record(self, model_class: Type[Base], data: Dict[str, Any], tenant_id: str = None) -> Optional[Base]:
        ...
        try:
            ...
            if hasattr(model_class, 'tenant_id') and 'tenant_id' not in data and tenant_id:
    ...
    async def get_by_id(self, model_class: Type[Base], record_id: Union[str, uuid.UUID], tenant_id: str = None) -> Optional[Base]:
    ...
    async def get_by_field(self, model_class: Type[Base], field_name: str, field_value: Any, tenant_id: str = None) -> Optional[Base]:
        ...
        try:
            ...
            logger.debug(f"Retrieved {model_class.__name__} by {field_name}: {field_value}")
            return record
    ...
    async def delete_record(self, model_class: Type[Base], record_id: Union[str, uuid.UUID], tenant_id: str = None) -> bool:
        ...
        try:
            ...
            logger.debug(f"Deleted {model_class.__name__} record: {record_id} - Success: {deleted}")
            return deleted
    ...
    async def count_records(self, model_class: Type[Base], filters: Dict[str, Any] = None, tenant_id: str = None) -> int:
    ...
    async def execute_raw_query(self, query: str, params: Dict[str, Any] = None, tenant_id: str = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if hasattr(model_class, search_field):
                ...
                query = select(model_class).where(
                    search_vector.op('@@')(func.plainto_tsquery('english', search_query))
                ).order_by(
                    func.ts_rank(search_vector, func.plainto_tsquery('english', search_query)).desc()
                ).limit(limit)
                
                result = await session.execute(query)
                records = result.scalars().all()
                
                logger.debug(f"Full-text search returned {len(records)} results for: {search_query}")
                return list(records)
            else:
                logger.warning(f"{model_class.__name__} does not have {search_field} field")
                return []
