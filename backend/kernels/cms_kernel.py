...
class CMSKernel(BaseKernel):
    ...
    async def create_page(
        self, tenant_id: str, page_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Create a new page"""
        async with self.connection_manager.get_session() as session:
            ...
            result = await session.execute(
                select(Page).where(
                    Page.tenant_id == tenant_id, Page.slug == page_data["slug"]
                )
            )
            ...
            if page_data.get("is_homepage"):
                await session.execute(
                    update(Page)
                    .where(Page.tenant_id == tenant_id, Page.is_homepage is True)
    ...
    async def update_page(
        self, page_id: str, tenant_id: str, update_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Update page"""
        async with self.connection_manager.get_session() as session:
    ...
    async def get_templates(
        self, industry_module: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """Get templates, optionally filtered by industry"""
        async with self.connection_manager.get_session() as session:
            query_conditions = [Template.is_active is True]
    ...
    async def create_template(self, template_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        async with self.connection_manager.get_session() as session:
            template_data.update({"is_active": True, "created_at": datetime.utcnow()})
    ...
    async def create_widget(
        self, tenant_id: str, widget_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Create a new widget"""
        async with self.connection_manager.get_session() as session:
            widget_data.update(
    ...
    async def get_widgets(
        self, tenant_id: str, widget_type: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """Get widgets for tenant"""
        async with self.connection_manager.get_session() as session:
            query_conditions = [Widget.tenant_id == tenant_id, Widget.is_active is True]
    ...
    async def upload_media(
        self, tenant_id: str, media_data: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def get_media_library(
        self, tenant_id: str, file_type: Optional[str] = None
    ) -> List[Dict[str, Any]]:
