...
class BookingKernel(BaseKernel):
    ...
    async def create_resource(
        self, tenant_id: str, resource_data: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def get_resources(
        ...
        async with self.connection_manager.get_session() as session:
            query_conditions = [
                ...
                Resource.is_active is True,
            ]

            result = await session.execute(select(Resource).where(*query_conditions))
            resources = result.scalars().all()
            return [resource.__dict__ for resource in resources]

    async def set_resource_availability(
        self, resource_id: str, availability_schedule: List[Dict[str, Any]]
    ):
    ...
    async def create_booking(
        self, tenant_id: str, booking_data: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def get_bookings(
        ...
        async with self.connection_manager.get_session() as session:
            query_conditions = [Booking.tenant_id == tenant_id]

            result = await session.execute(
                select(Booking).where(*query_conditions).order_by(Booking.start_time)
            )
