...
def upgrade() -> None:
    ...
    op.create_table('tenants',
        ...
        sa.Column('branding', postgresql.JSONB(astext_type=sa.Text()), nullable=True, default=sa.text("'{}'::jsonb")),
        sa.Column('settings', postgresql.JSONB(astext_type=sa.Text()), nullable=True, default=sa.text("'{}'::jsonb")),
        sa.Column('feature_toggles', postgresql.JSONB(astext_type=sa.Text()), nullable=True, default=sa.text("'{}'::jsonb")),
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
        sa.Column('updated_at', sa.DateTime(timezone=True), nullable=True),
        sa.PrimaryKeyConstraint('id')
    )
    op.create_index('idx_tenants_subdomain', 'tenants', ['subdomain'], unique=True)
    op.create_index('idx_tenants_industry_active', 'tenants', ['industry_module', 'is_active'])
    op.create_index('idx_tenants_branding_gin', 'tenants', ['branding'], postgresql_using='gin')
    op.create_index('idx_tenants_settings_gin', 'tenants', ['settings'], postgresql_using='gin')
    ...
    op.create_table('users',
        ...
        sa.Column('profile', postgresql.JSONB(astext_type=sa.Text()), nullable=True, default=sa.text("'{}'::jsonb")),
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
        sa.Column('updated_at', sa.DateTime(timezone=True), nullable=True),
    ...
    op.create_index('idx_users_profile_gin', 'users', ['profile'], postgresql_using='gin')
    ...
    op.create_table('pages',
        ...
        sa.Column('content_blocks', postgresql.JSONB(astext_type=sa.Text()), nullable=True, default=sa.text("'[]'::jsonb")),
    ...
    op.create_index('idx_pages_content_gin', 'pages', ['content_blocks'], postgresql_using='gin')
    ...
    op.create_table('leads',
        ...
        sa.Column('custom_fields', postgresql.JSONB(astext_type=sa.Text()), nullable=True, default=sa.text("'{}'::jsonb")),
    ...
    op.create_index('idx_leads_custom_fields_gin', 'leads', ['custom_fields'], postgresql_using='gin')
    ...
    op.create_table('forms',
        ...
        sa.Column('success_message', sa.Text(), nullable=True, default='Thank you for your submission!'),
        sa.Column('redirect_url', sa.String(length=500), nullable=True),
        sa.Column('is_active', sa.Boolean(), nullable=True, default=True),
        sa.Column('fields', postgresql.JSONB(astext_type=sa.Text()), nullable=True, default=sa.text("'[]'::jsonb")),
        sa.Column('email_notifications', postgresql.JSONB(astext_type=sa.Text()), nullable=True, default=sa.text("'[]'::jsonb")),
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
        sa.Column('updated_at', sa.DateTime(timezone=True), nullable=True),
        sa.ForeignKeyConstraint(['tenant_id'], ['tenants.id'], ),
        sa.PrimaryKeyConstraint('id'),
    ...
    op.create_index('idx_form_submissions_form_created', 'form_submissions', ['form_id', 'created_at'])
    op.create_index('idx_form_submissions_data_gin', 'form_submissions', ['data'], postgresql_using='gin')
    ...
    op.create_table('templates',
        ...
        sa.Column('is_active', sa.Boolean(), nullable=True, default=True),
        sa.Column('layout_config', postgresql.JSONB(astext_type=sa.Text()), nullable=True, default=sa.text("'{}'::jsonb")),
        sa.Column('default_content', postgresql.JSONB(astext_type=sa.Text()), nullable=True, default=sa.text("'{}'::jsonb")),
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
        sa.Column('updated_at', sa.DateTime(timezone=True), nullable=True),
        sa.PrimaryKeyConstraint('id')
    )
    op.create_index('idx_templates_industry_active', 'templates', ['industry_module', 'is_active'])
    op.create_index('idx_templates_layout_gin', 'templates', ['layout_config'], postgresql_using='gin')
    ...
    op.create_index('idx_widgets_config_gin', 'widgets', ['config'], postgresql_using='gin')
    ...
    op.create_index('idx_tour_slots_staff_available', 'tour_slots', ['staff_user_id', 'is_available'])
    ...
    op.create_table('tours',
        ...
        sa.ForeignKeyConstraint(['staff_user_id'], ['users.id'], ),
        sa.ForeignKeyConstraint(['tenant_id'], ['tenants.id'], ),
        sa.ForeignKeyConstraint(['tour_slot_id'], ['tour_slots.id'], ),
        sa.PrimaryKeyConstraint('id')
    )
    op.create_index('idx_tours_tenant_scheduled', 'tours', ['tenant_id', 'scheduled_at'])
    ...
    op.create_index('idx_pages_search_gin', 'pages', ['search_vector'], postgresql_using='gin')
    ...
    op.execute("GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO application_role")
    op.execute("GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA public TO application_role")
