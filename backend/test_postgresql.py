...
async def test_postgresql_setup():
    ...
    try:
        ...
        async with connection_manager.get_session() as session:
            ...
            existing_tenant = await identity_kernel.get_tenant_by_subdomain(
                "test-tenant"
            )
            if existing_tenant:
            ...
            existing_user = await identity_kernel.get_user_by_email(
                tenant["id"], "test@example.com"
            )
            if existing_user:
                logger.info("ℹ️ Test user already exists")
                user = existing_user
            else:
                user = await identity_kernel.create_user(
                    tenant["id"], user_data, "test123"
                )
