...
class DatabaseOptimizer:
    ...
    async def optimize_query(
        ...
        try:
            async with self.connection_manager.get_session() as session:
                ...
                await self.log_query_performance(
                    table_name, "select", len(data), execution_time
                )
            ...
            execution_time = (time.time() - start_time) * 1000
            await self.log_query_performance(
                table_name, "select", 0, execution_time, str(e)
            )
    ...
    async def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_queries": sum(
                m["total_queries"] for m in self.query_metrics.values()
            ),
            "total_slow_queries": sum(
                m["slow_queries"] for m in self.query_metrics.values()
            ),
            ...
            async with self.connection_manager.get_session() as session:
                ...
                return {
                    ...
                    "statistics": {
                        ...
                        "last_vacuum": (
                            stats.last_vacuum.isoformat() if stats.last_vacuum else None
                        ),
                        "last_analyze": (
                            stats.last_analyze.isoformat()
