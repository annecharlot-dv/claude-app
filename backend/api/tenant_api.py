...
async def create_tenant(
    ...
    if not await tenant_service.validate_subdomain_available(request.subdomain):
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT, detail="Subdomain already exists"
        )
...
async def list_tenants(
    ...
    try:
        tenants = await tenant_repo.list_tenants(
            status=status, industry=industry, limit=limit, offset=offset
        )
...
async def check_subdomain_availability(
    subdomain: str, tenant_service: TenantService = Depends(get_tenant_service)
):
    ...
    return SubdomainCheckResponse(
        subdomain=subdomain, available=available, suggestions=suggestions
    )


# Tenant-Specific Endpoints (require tenant context)
@router.get("/current", response_model=TenantDetailResponse)
async def get_current_tenant(tenant: TenantModel = Depends(get_tenant_from_request)):
...
async def get_tenant_health(tenant: TenantModel = Depends(get_tenant_from_request)):
