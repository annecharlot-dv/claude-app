...
def require_role(required_roles: List[UserRole]):
    async def role_checker(current_user: User = Depends(get_current_user)):
        ...
        has_permission = await core.check_user_permission(
            current_user.tenant_id, current_user.id, f"role.{user_role_str}"
        )
...
async def register_user(user_data: UserCreate, tenant_subdomain: str):
    ...
    existing_user = await identity_kernel.get_user_by_email(
        tenant["id"], user_data.email
    )
    if existing_user:
    ...
    access_token = await identity_kernel.create_access_token(
        created_user["id"], timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES)
    )
...
async def login_user(user_data: UserLogin, tenant_subdomain: str):
    ...
    auth_result = await identity_kernel.authenticate_user(
        tenant_subdomain, user_data.email, user_data.password
    )
    ...
    # Create access token
    access_token = await identity_kernel.create_access_token(
        auth_result["id"], timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES)
    )
...
async def create_default_homepage(tenant_id: str, industry_module: IndustryModule):
    ...
    async with connection_manager.get_session() as session:
        result = await session.execute(
            select(Template).where(Template.industry_module == industry_module)
        )
...
async def get_page(page_id: str, current_user: User = Depends(get_current_user)):
    ...
    async with connection_manager.get_session() as session:
        result = await session.execute(
            select(Page).where(
                Page.id == page_id, Page.tenant_id == current_user.tenant_id
            )
        )
...
async def submit_form(form_id: str, submission: FormSubmission, request: Request):
    ...
    for field in form_obj.fields:
        if field.is_required and field.label.lower() not in [
    ...
    lead_data = {
        ...
        "first_name": submission.data.get(
            "first_name", submission.data.get("name", "Unknown")
        ),
    ...
    if existing_lead:
        ...
        await session.execute(
            update(Lead)
            .where(Lead.id == existing_lead.id)
            .values(
                updated_at=datetime.utcnow(),
                custom_fields={
                    **(existing_lead.custom_fields or {}),
...
async def get_tours(
    ...
    async with connection_manager.get_session() as session:
        result = await session.execute(
            select(Tour)
            .where(Tour.tenant_id == current_user.tenant_id)
            .order_by(Tour.scheduled_at)
        )
...
async def get_dashboard_stats(
    ...
    async with connection_manager.get_session() as session:
        ...
        conversion_rate = (
            (converted_leads / new_leads_this_month * 100)
            if new_leads_this_month > 0
            else 0
        )
...
async def reload_tenant_module(
    current_user: User = Depends(
        require_role([UserRole.ACCOUNT_OWNER, UserRole.ADMINISTRATOR])
    )
):
...
async def save_page_builder_data(
    ...
    success = await cms_engine.save_page_builder_data(
        current_user.tenant_id, page_id, blocks_data.get("blocks", [])
    )
...
async def get_page_builder_data(
    ...
    builder_data = await cms_engine.get_page_builder_data(
        current_user.tenant_id, page_id
    )
    ...
    if builder_data:
        ...
        return {"page_id": page_id, "blocks": [], "updated_at": None}
...
async def get_performance_metrics(
    hours: int = 1,
    current_user: User = Depends(
        require_role([UserRole.PLATFORM_ADMIN, UserRole.ACCOUNT_OWNER])
    ),
):
    ...
    if current_user.role != UserRole.PLATFORM_ADMIN:
        if tags:
            tags = [
                tag
                for tag in tags
                if tag.startswith(f"tenant:{current_user.tenant_id}")
            ]
        else:
...
async def health_check():
    ...
    try:
        ...
        return {
            ...
            "database": "connected",
            "performance": {
                "avg_response_time": metrics_summary.get("metrics", {})
                .get("response_time", {})
                .get("avg", 0),
